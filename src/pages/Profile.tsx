...
import { BarChart, Briefcase, Package, Calendar, Clock, CheckCircle2, XCircle, Clock3, ExternalLink, UserCircle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
        <div className="bg-gradient-to-r from-black to-gray-800 rounded-2xl p-6 sm:p-8 mb-6 sm:mb-12 text-white">
          <h1 className="text-2xl sm:text-3xl font-bold mb-2 break-words">{user?.email}</h1>
          <p className="text-gray-300 text-sm sm:text-base">Manage your job applications and profile settings</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8 mb-6 sm:mb-12">
            ...
            {subscription ? (
              <div>
                <div className="flex items-baseline space-x-2 mb-4">
                  <span className="text-2xl sm:text-3xl font-bold">{subscription.plan_name}</span>
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-semibold">Applications</h3>
                <p className="text-xs sm:text-sm text-gray-500">Total Submissions</p>
              </div>
            </div>
            <div className="text-2xl sm:text-3xl font-bold mb-4">{applications.length}</div>
            {subscription && (
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-gray-500">Latest Application</p>
              </div>
            </div>
            <div className="h-[120px]">
              {applications.length > 0 ? (
                <div className="p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <p className="font-medium text-black text-sm sm:text-base truncate">{applications[0].job_title}</p>
                  <p className="text-gray-600 text-xs sm:text-sm truncate">{applications[0].company_name}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="p-6 sm:p-8 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h3 className="text-lg sm:text-xl font-semibold">
                {showProfileForm ? 'Profile Settings' : 'Job Applications'}
              </h3>
              <p className="text-sm text-gray-500">
                {showProfileForm ? 'Update your personal information' : 'Track all your job applications'}
              </p>
            </div>
            <label className="inline-flex items-center cursor-pointer">
              <div className="relative">
                <input
                  type="checkbox"
                  className="sr-only"
                  checked={showProfileForm}
                  onChange={() => setShowProfileForm(!showProfileForm)}
                />
                <div className={`block w-14 h-8 rounded-full transition-colors duration-300 ${
                  showProfileForm ? 'bg-black' : 'bg-gray-200'
                }`}>
                  <div className={`absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform duration-300 ${
                    showProfileForm ? 'transform translate-x-6' : ''
                  }`}>
                    <UserCircle className={`h-6 w-6 ${showProfileForm ? 'text-black' : 'text-gray-400'}`} />
                  </div>
                </div>
              </div>
              <span className="ml-3 text-sm font-medium text-gray-700">
                {showProfileForm ? 'Show Applications' : 'Show Profile'}
              </span>
            </label>
          </div>
                  ...
                  {applications.length > 0 ? (
                    <table className="min-w-full divide-y divide-gray-100">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="text-left p-4 sm:py-4 sm:px-6 text-xs sm:text-sm font-medium text-gray-500">Job Title</th>
                          <th className="text-left p-4 sm:py-4 sm:px-6 text-xs sm:text-sm font-medium text-gray-500">Company</th>
                          <th className="text-left p-4 sm:py-4 sm:px-6 text-xs sm:text-sm font-medium text-gray-500">Status</th>
                          <th className="text-left p-4 sm:py-4 sm:px-6 text-xs sm:text-sm font-medium text-gray-500">Applied Date</th>
                          <th className="text-left p-4 sm:py-4 sm:px-6 text-xs sm:text-sm font-medium text-gray-500">Job Link</th>
                        </tr>
                      </thead>
                        ...
                        {applications.map((app) => {
                          ...
                          return (
                            <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                              <td className="p-4 sm:py-4 sm:px-6">
                                <div className="font-medium text-sm sm:text-base">{app.job_title}</div>
                              </td>
                              <td className="p-4 sm:py-4 sm:px-6 text-gray-600 text-sm">{app.company_name}</td>
                              <td className="p-4 sm:py-4 sm:px-6">
                              ...
                              <td className="p-4 sm:py-4 sm:px-6 text-gray-600 text-sm">
